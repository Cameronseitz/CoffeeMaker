...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( recipe.getIngredients().isEmpty() ) {
            return new ResponseEntity( errorResponse( "No ingredients in recipe" ), HttpStatus.BAD_REQUEST );
        }
     ...
     * provided to a Recipe object and using that to update the one found by the
     * passed in name. Invalid JSON will fail.
     *
     * @param recipe
     *            The valid Recipe to be used to update the fields of the one
     *            given by the name.
     * @param name
     *            recipe name
     * @return ResponseEntity indicating success if the Recipe could be saved to
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe, @PathVariable ( "name" ) final String name ) {
        if ( null == service.findByName( name ) ) {
            return new ResponseEntity( errorResponse( "Recipe with the name " + name + " cannot be found" ),
                    HttpStatus.NOT_FOUND );
        }
        if ( recipe.getIngredients().isEmpty() ) {
            return new ResponseEntity( errorResponse( "No ingredients in recipe" ), HttpStatus.BAD_REQUEST );
        }
        final Recipe recipeToUpdate = service.findByName( name );
        recipeToUpdate.updateRecipe( recipe );
        service.save( recipeToUpdate );
